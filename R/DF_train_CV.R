...
#'
#'   CV_result = DF_train_CV(Train_X, Train_Y, CV_fold=10, stop_step = 4, Method = "MCC")
#'
#'
DF_train_CV = function (X, Y, CV_fold=5,stop_step=4,
                        Max_tree=20,Discard='All',
                        Filter = F , p_val = 0.05, Method = "ACC", Quiet = T,
  ...
  for ( CV_iter in 1:CV_fold){
    ...
    # Calculating MIS
    model = Con_DT(Train_data,Train_label)
    ...
    while(Curr_tree<Max_tree & stop_signal<=stop_step & length(used_feat)>1 ){
      ...
      if (Discard == "All"){
        ...
        used_var = feat_used[1:min(length(feat_used),round(Discard))]
      }else{
        if (Quiet == F){
  ...
  result = list(performance=Fin_performance,pred=pred_all,detail=Tree_model,Method=Method)
  return(result)
}
