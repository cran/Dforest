  ...
  if (plot == T){
    ...
    p=ggplot(plot_matrix)+
      ...
      scale_y_continuous(name = "Accuracy", breaks = c(0,20,40,60,80,100),
                         minor_breaks = c(10,30,50,70,90),limits = c(0,100))+
      geom_label(aes(x=min(ACC_Conf[,1])+0.03, y=95, label="Samples (%)"),size = 5, fill = "firebrick4",color ="white")+
      theme(axis.title.y = element_text(color = "Blue", face="bold", size = 15))+
      xlim(min(ACC_Conf[,1]),1)
...
DF_ConfPlot_accu = function (Pred_result, Label, bin=20, plot = T, smooth = F){
  ...
  if (plot == T){
    ...
    p=ggplot(plot_matrix)+
      ...
      scale_y_continuous(name = "Accuracy", breaks = c(0,20,40,60,80,100),
                         minor_breaks = c(10,30,50,70,90),limits = c(0,100))+
      geom_label(aes(x=min(ACC_Conf[,1])+0.03, y=20, label="Samples (%)"),size = 5, fill = "firebrick4",color ="white")+
      theme(axis.title.y = element_text(color = "Blue", face="bold", size = 15))+
      xlim(min(ACC_Conf[,1]),1)
    if (smooth == T){
